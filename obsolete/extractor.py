    ...
    for root, _, files in os.walk(song_directory):
        for file in files:
            if file.lower().endswith((scrape_type)):
                ...
                shutil.copy2(image_path, output_directory)
                 # Get the original file name without the extension
                #file_name, file_extension = os.path.splitext(file)
            
                # New file name
                #print(image_path)
                #new_file_name = get_song_name_from_qua(os.path.join(image_path)) + file_extension
            
                # New path for the renamed file
                #new_file_path = os.path.join(output_directory, new_file_name)
            
                # Rename the copied file
                #os.rename(os.path.join(output_directory, file), new_file_path)
